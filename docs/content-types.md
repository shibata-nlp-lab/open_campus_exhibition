...
間隔は単語ごとに決められます（`autoFocusSecs`。並びは `autoTokenIndexes` と同じ順で、
足りないところは `autoFocusSec`）。最後の単語まで見せたあと次のコンテンツへ進むときだけ
`screenAutoSec.vectors` を使い、音声の終わりを待ちます。

見せる単語も時間も「1, 3, 5」のように打ち込みます（[src/lib/numberList.ts](../src/lib/numberList.ts)）。
位置は 1 起点で受けて 0 起点に直し、秒数はずらさず **0 も残します**（0 = 待たずに次へ）。
**保存する数値の配列から毎回文字列を作り直すと、区切りを打った瞬間に消えて2つ目以降を打てません**
（`"1, "` → `[0]` → `"1"`）。入力欄は打っている間の文字列を自分で持ち、保存だけ数値に直します。
