...
**1 回の実行で 1 レース**ぶんを作ります。上から順に実行すると、
`races/races_01.csv` ができます（Colab なら `/content/races/races_01.csv`）。

4 章に戻って `RACE_NO` と `PROMPT` を変えて回すと `races_02.csv` … と増えていきます。
溜まったら、アプリの設定画面「コンテンツ」→ 馬券風 次単語予想 →
**「フォルダごと取り込む…」で `races` フォルダを選ぶ**だけで全部入ります。
取り込みは追加なので、あとから作ったレースを足していけます。
...
**Colab には日本語フォントが無いので、その場で入れて登録します**（ランタイムの再起動は要りません）。
それでも □ になる場合は、フォントのセルをもう一度実行してください。
